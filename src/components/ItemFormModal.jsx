...
export default function ItemFormModal({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Only send relevant fields for shopping/pantry
            ...
            <Dialog.Panel className="w-full max-w-md bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border border-white/30">
              <div className="flex items-center justify-between p-6 border-b border-slate-200">
                <Dialog.Title className="text-lg font-semibold text-slate-800">
                  {initialData ? (isShopping ? 'Edit Shopping Item' : 'Edit Pantry Item') : (isShopping ? 'Add Shopping Item' : 'Add Pantry Item')}
                </Dialog.Title>
                <button
                  onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200 ${errors.name ? 'border-danger-300' : 'border-slate-300'}`}
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.1"
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200 ${errors.quantity ? 'border-danger-300' : 'border-slate-300'}`}
                    required
                  />
                  {errors.quantity && <p className="mt-1 text-sm text-danger-600">{errors.quantity}</p>}
                </div>
                {/* Category Field for Shopping List */}
                {isShopping && (
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200 ${errors.category ? 'border-danger-300' : 'border-slate-300'}`}
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200 ${errors.expiry_date ? 'border-danger-300' : 'border-slate-300'}`}
                      />
                      {errors.expiry_date && <p className="mt-1 text-sm text-danger-600">{errors.expiry_date}</p>}
                    </div>
                    <div>
